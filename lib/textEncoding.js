...
function looksCorruptedText(text) {
  ...
  // URL query (?startDate=...) dễ khớp nhầm pattern mojibake — loại trước khi kiểm tra.
  const probe = value.replace(/\bhttps?:\/\/\S+/gi, " ");
  // UTF-8 đọc nhầm Latin-1 hoặc ký tự thay thế — không dùng /i vì sẽ match nhầm "ã" trong tiếng Việt.
  if (/Ã.|Ä.|â€|ï¿½|\uFFFD/.test(probe)) return true;
  ...
  if (/áº|á»|Æ°|lÆ°|giA|trA/.test(probe)) return true;
  if (/có vớ/i.test(probe)) return true;
  if (/gì[\s\S]{0,4}óy\s*$/i.test(probe)) return true;
  // Decode mất dấu: ký tự ? thay cho chữ có dấu.
  if (/[a-zA-Z]\?[a-zA-ZÀ-ỹ]/i.test(probe) || /[À-ỹ]\?[a-zA-Z]/i.test(probe)) return true;
  if (/\b(?:b|t|d|n|m|h|r|l|g|v|p|c|s|u|o|a|e|i)\?[a-z]{1,4}\b/i.test(probe)) return true;
  return false;
}
...
const INTERNAL_CURSOR_PROMPT_PATTERNS = [
  /^Briefly inform the user about the task result/i,
  /^Continue working on the task/i,
  /^You are running in a loop/i,
  /^The user has sent the following message/i,
];

function isInternalCursorPrompt(text) {
  const value = normalizeQueryText(text);
  if (!value) return true;
  return INTERNAL_CURSOR_PROMPT_PATTERNS.some((pattern) => pattern.test(value));
}

function filterUserQueryTexts(queries) {
  return (queries || [])
    .map((query) => normalizeQueryText(query))
    .filter((query) => query && !looksCorruptedText(query) && !isInternalCursorPrompt(query));
}

function filterUserQueryEvents(events) {
  return (events || [])
    .map((row) => ({
      text: normalizeQueryText(row?.text),
      at: row?.at ? new Date(row.at) : null,
    }))
    .filter((row) => row.text && !looksCorruptedText(row.text) && !isInternalCursorPrompt(row.text) && row.at);
}
...
function hasActivityOnOrAfter(events, sinceMs = startOfTodayMs()) {
