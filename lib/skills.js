...
// (repo nguồn, publish lên Git) và .agents/skills/<slug>/SKILL.md (project-local) để
// Cursor dùng được qua "/<slug>" trong repo này. Không cài global (~/.cursor/skills/).
...
// Cài skill vào .agents/skills/<slug>/SKILL.md (project-local) để Cursor nhận ngay
// trong repo quoc-agent-skills. Project khác cài qua `npx skills add ...` từ Git.
function installToCursorProject(slug, description, body) {
  try {
    const created = writeSkillMarkdown(
      path.join(CURSOR_PROJECT_SKILLS_DIR, slug),
      slug,
      description,
      body
    );
    if (created) {
      console.log(`✅ Đã cài skill "${slug}" vào .agents/skills/${slug} - dùng được qua "/${slug}" trong project này`);
    }
  } catch (err) {
    console.error(`Không cài được skill "${slug}" vào .agents/skills:`, err.message);
  }
}
...
async function createSkill({ name, description, content, slug: slugInput }) {
  ...
  installToCursorProject(slug, desc, body);
