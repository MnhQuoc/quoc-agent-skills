...
// xóa khỏi MongoDB (và khỏi .agents/skills/<slug> nếu đã được cài ở đó).
//
...
// (skills/<slug>/SKILL.md) thì bị xóa khỏi DB + khỏi .agents/skills/<slug> (nếu có).
async function syncDeletedSkills() {
  ...
  for (const { slug } of dbSkills) {
    ...
    try {
      ...
      // bỏ qua nếu không xóa được bản project-local - không quan trọng bằng MongoDB
    }
